    ...
    if ((!(_sector in KPLIB_sectors_player)) && (([markerPos _sector, [_opforcount, _sector] call KPLIB_fnc_getSectorRange, KPLIB_side_player] call KPLIB_fnc_getUnitsCount) > 0)) then {
        ...
        while {!_stopit} do {
            // Define min players by sector type
            private _minPlayers = 2;
            switch (true) do {
            ...
            // Get current range for sector
            private _opforcount = [] call KPLIB_fnc_getOpforCap;  
            private _range = [_opforcount, _sector] call KPLIB_fnc_getSectorRange;  


            // Get players inside sector range
            private _playerUnits = allPlayers select {
                (_x distance2D markerPos _sector) < _range
            };
            private _playerCount = count _playerUnits;

            // Get friendly AI (side == KPLIB_side_player) inside sector range, exclude players
            private _aiUnits = allUnits select {
                side _x == KPLIB_side_player &&
                !(_x isKindOf "Man" && {_x in allPlayers}) &&
                (_x distance2D markerPos _sector) < _range
            };
            private _aiCount = count _aiUnits;

            private _totalCount = _playerCount + _aiCount;
            ...
            if (_totalCount < _minPlayers) then {
                ...
                // Kill players with lightning
                {
                    private _playerpos = getPosATL _x;
                    diag_log _playerpos;
                    [_playerpos, nil, true] call BIS_fnc_moduleLightning;
                } forEach _playerUnits;

                // Kill AI with lightning and teleport to nearest FOB
                {
                    private _aipos = getPosATL _x;
                    diag_log _aipos;
                    [_aipos, nil, true] call BIS_fnc_moduleLightning;
                } forEach _aiUnits;
                // Skip capture logic this tick
